...
def InfoLog(message):
    ...
    logging.basicConfig(filename='./log/'+datetime.datetime.now().strftime("%Y-%m-%d")+'.info.log', level=logging.INFO , format=format)
    logging.info(message)

def ErrorLog(message):
    #format='%(asctime)s - %(pathname)s - %(filename)s - [line:%(lineno)d] - %(levelname)s - %(message)s'
    format='%(asctime)s %(filename)s[line:%(lineno)d] %(levelname)s %(message)s'
    logging.basicConfig(filename='./log/'+datetime.datetime.now().strftime("%Y-%m-%d")+'.error.log', level=logging.ERROR , format=format)
            ...
            if seconds >= frequence:
                try:
                    ...
                    if content:
                        ...
                        #print ("heheheheheheehhe")
                        if response_status  == response :
                            #print ("hahahahahaha4444444444")
                            message = url+"访问正常,响应时间:"+str(respseconds)+"ms"
                            #print(message)
                            InfoLog(message)
                        else:
