...
def select_cluster(req):
    ...
    if flag == '1':
        ...
        sql_1 = "select user_id,mobile,mms_id,insert_time,status,result,fail_reason from mms_submit_cdma where user_id= "
        ...
        if startTime!=endTime:
            if startTime!='':
                sql_1 +=" and msg_receive_time >='"+startTime+"'"
                sql_2 +=" and msg_receive_time >='"+startTime+"'"
                sql_3 +=" and msg_receive_time >='"+startTime+"'"
                sql_4 +=" and msg_receive_time >='"+startTime+"'"
            if endTime!='':
                sql_1 +=" and msg_receive_time <='"+endTime+"'"
                sql_2 +=" and msg_receive_time <='"+endTime+"'"
                sql_3 +=" and msg_receive_time <='"+endTime+"'"
                sql_4 +=" and msg_receive_time <='"+endTime+"'"
        else:
            if startTime!='':
                sql_1 +=" and msg_receive_time like '"+startTime+"%'"
                sql_2 +=" and msg_receive_time like '"+startTime+"%'"
                sql_3 +=" and msg_receive_time like '"+startTime+"%'"
                sql_4 +=" and msg_receive_time like '"+startTime+"%'"
...
def export_cluster(req):
    ...
    print (sql)
    try:
        conn,cur=ShareMethod.views.connDB_yw('210.14.134.81',dbname)
        ShareMethod.views.exeQuery(cur,sql)
        ShareMethod.views.connClose(conn,cur)
